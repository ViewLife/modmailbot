...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxServerCommand("snippet", "<trigger> [text$]", async (msg, args, thread) => {
    ...
    if (snippet) {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Snippet "${args.trigger}" existe déjà! Vous pouvez le modifier ou le supprimer avec ${config.prefix}edit_snippet et ${config.prefix}delete_snippet respectivement.`);
      } else {
  ...
  commands.addInboxServerCommand("edit_snippet", "<trigger> <text$>", async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Snippet "${args.trigger}" édité !`);
  }, {
    aliases: ["es"]
  });
  ...
  commands.addInboxServerCommand("snippets", [], async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Snippets disponible (prefix ${config.snippetPrefix}):\n${triggers.join(", ")}`);
  }, {
    aliases: ["s"]
  });
};
